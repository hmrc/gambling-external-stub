...
class GamblingPaymentsControllerSpec extends AnyWordSpec with Matchers with SpecBase {
  ...
  private val controller = app.injector.instanceOf[GamblingPaymentsController]
  ...
  "GamblingPaymentsController#getPayments" should {
    ...
    "return BAD_REQUEST for an unrecognised regime" in {
      val result = controller.getPayments("INVALID", "XWM00003103200", 1, 10)(FakeRequest())
    ...
    "accept all valid regimes (case-insensitive)" in {
      Seq("MGD", "mgd", "GBD", "gbd", "PBD", "pbd", "RGD", "rgd").foreach { regime =>
        val result = controller.getPayments(regime, "XWM00003100200", 1, 10)(FakeRequest())
        status(result) shouldBe OK
      }
    }

    "return BAD_REQUEST for XWM00003100400 (last 3 digits = 400)" in {
      val result = controller.getPayments("MGD", "XWM00003100400", 1, 10)(FakeRequest())
    ...
    "return UNAUTHORIZED for XWM00003100401 (last 3 digits = 401)" in {
      val result = controller.getPayments("MGD", "XWM00003100401", 1, 10)(FakeRequest())
    ...
    "return NOT_FOUND for XWM00003100404 (last 3 digits = 404)" in {
      val result = controller.getPayments("MGD", "XWM00003100404", 1, 10)(FakeRequest())
    ...
    "return INTERNAL_SERVER_ERROR for XWM00003100500 (last 3 digits = 500)" in {
      val result = controller.getPayments("MGD", "XWM00003100500", 1, 10)(FakeRequest())
    ...
    "return 3 records for XWM00003103200 (last 3 = 200, 4th+5th from right = 03)" in {
      val result = controller.getPayments("MGD", "XWM00003103200", 1, 10)(FakeRequest())
    ...
    "return first page for XWM00003109200 (9 records) with pageNo=1 pageSize=5" in {
      val result = controller.getPayments("MGD", "XWM00003109200", 1, 5)(FakeRequest())
    ...
    "return second page for XWM00003109200 (9 records) with pageNo=2 pageSize=5" in {
      val result = controller.getPayments("MGD", "XWM00003109200", 2, 5)(FakeRequest())
    ...
    "return 50 total records for XWM00003150200 with pageNo=1 pageSize=10" in {
      val result = controller.getPayments("MGD", "XWM00003150200", 1, 10)(FakeRequest())
    ...
    "return last page for XWM00003150200 with pageNo=5 pageSize=10" in {
      val result = controller.getPayments("MGD", "XWM00003150200", 5, 10)(FakeRequest())
